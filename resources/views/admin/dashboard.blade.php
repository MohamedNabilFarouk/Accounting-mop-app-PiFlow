...
<div class="card rounded mb-5 mb-xl-8 shadow-lg">
    ...
    <div class="card-header rounded border-0 pt-5">
        ...
        <div class="card-toolbar">
            <a href="{{route('company.create')}}" class="btn btn-sm btn-light-primary">
            ...
            <!--end::Svg Icon-->@lang('add') @lang('Company') +</a>
        </div>
        @endrole
    </div>
    ...
    <div class="card-body py-3">
        ...
        <div class="pull-right search"><input class="form-control" type="text" name='name' value="{{ Request('name') }}" placeholder="Account Name"></div>
        </div>
        <div class="col-md-2 mb-4">
        <div class="pull-right search">
           
             <select class="bs-select form-control" name='status' tabindex="-98">
            <option value=''  @if(Request('status') == null ) selected @endif>Select Status</option>
            ...
            <div class="pull-right search"><label>Near to expire</label> <input type="checkbox"   name="expire" value='1' class="expire-checkbox" @if(Request('expire') == 1 ) checked @endif /></div>
            ...
            <table class="table table-hover align-middle gs-0 gy-4">
                ...
                <thead>
                    <tr class="text-center border-3 fw-bolder text-muted bg-light">
                        ...
                        <th class="min-w-125px">@lang('tax register')</th>
                        <th class="min-w-125px">@lang('com register')</th>
...
{{-- @dd($users[0]->company); --}}
                @foreach($companies as $index => $c)
                    <tr class="text-center  border-3 m-auto">
                         ...
                         <td class="px-3">
                            <div class="d-flex align-items-center">
                                    <span class="fw-bold  d-block fs-7">{{$c->tax_register}}</span>
                                </div>
                            </div>
                        </td>
                         <td class="px-3">
                            <div class="d-flex align-items-center">
                                    <span class="fw-bold  d-block fs-7">{{$c->com_register}}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-3">
                            <span class='status{{ $c->id }}'></span>
                                    ...
                                    <span class="switch switch-icon">
                                        <label>
                                            <input type="checkbox" @if($c->status['text'] == 'Active') checked="checked" @endif  class="user-checkbox" data-company-id="{{ $c->id }}" />
...
@endrole
                                ...
                                <td class="px-3">
                                    <span class="badge  @if( Carbon\Carbon::parse(@$c->subscription_to)->diffInDays(Carbon\Carbon::now()) <= 7) badge-light-danger @else badge-light-primary @endif fs-7 fw-bold">@if( @$c->subscription_to != null){{ Carbon\Carbon::parse($c->subscription_to ?? NULL)->format('Y-m-d') }} @else - @endif</span>
                        ...
                        <td class="text-start">
                            <a href="{{ route('user.dates', $c->id ??0) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                <i class="fa fa-eye"></i>
                            </a>
                            <a href="{{ route('company.account', $c->id ??0) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                <i class="fa fa-users"></i>
                            </a>
                            @role('Piflow Admin')
                            <a href="{{ route('company.edit', $c->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
