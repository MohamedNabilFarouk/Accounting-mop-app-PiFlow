...
<div class="card rounded mb-5 mb-xl-8 shadow-lg">
    ...
    <div class="card-header rounded border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">@lang('Justifications')</span>
        </h3>
        <div class="card-toolbar">
            <form id="myForm" action="{{ route('justification.create') }}" method="get">
            ...
            <!--end::Svg Icon-->@lang('add') @lang('Justification') +</a>
        </div>
    </div>
    <!--end::Header-->
    <!--begin::Body-->
            ...
            <table class="table table-hover align-middle gs-0 gy-4">
                ...
                <thead>
                    <tr class="text-center border-3 fw-bolder text-muted bg-light">
                        {{-- <th class="min-w-125px">@lang('Icon')</th> --}}
                        <th class="min-w-125px">@lang('Bank Name')</th>
                        <th class="min-w-125px">@lang('Bank Number')</th>
                        <th class="min-w-125px">@lang('Piflow Comment')</th>
                        <th class="min-w-125px " >@lang('Client Comment')</th>
                        <th class="min-w-125px">@lang('Description')</th>
                        <th class="min-w-125px">@lang('Date')</th>
                    ...
                    <tr class="text-center border-3 m-auto">
                        ...
                        <td class="px-3">
                            <div class="d-flex align-items-center">
                                    <span class="text-muted fw-bold text-muted d-block fs-7">{{$c->bank_name}}</span>
                                </div>
                            </div>
                        </td>

                         <td class="px-3">
                                    <span class="badge badge-light-primary fs-7 fw-bold">{{$c->bank_number}}</span>
                        </td>
                         <td class="px-3">
                                   {{$c->piflow_comment}}
                        </td>
                         <td class="px-3 multiline-td  text-break">
                                   {{ $c->client_comment }}
                        </td>
                         <td class="px-3 multiline-td">
                                   {{$c->des}}
                        </td>
                         <td class="px-3">
                                    <span class="badge badge-light-primary fs-7 fw-bold">{{$c->trans_date}}</span>
                        </td>
                        ...
                        <td class="text-start">
                            <form id="files{{ $key }}" action="{{ route('user.files') }}" method="get">
                                ...
                                <input type="hidden" name="company_id" value="{{ $req['company_id'] }}">
                                <input type="hidden" name="justification_id" value="{{ $c->id }}">
                                <input type="hidden" name="date" value="{{ $req['date'] }}">
                            ...
                            <a href="{{ route('justification.edit',$c->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
